/*
 * ESP32S3 AP mode - Web page manager
 *
 * This sketch turns an ESP32-S3 board into a small web server that:
 ...
 *      "/on" and "/off" commands to toggle the built-in LED, over both
...
#include <WiFi.h>       // ESP32 core WiFi library: provides WiFi.mode(), WiFi.softAP(),
                         // WiFi.begin(), WiFiServer, WiFiClient, and related networking classes
#include "FreeRTOS.h"   // FreeRTOS core definitions (task types, tick macros, etc.)
#include "task.h"       // FreeRTOS task API: xTaskCreate(), vTaskDelay(), etc.
...
void initWiFi() {
  ...
  for (int i=0;i<2;i++) {  // Retry connecting to the station network up to 2 times
    ...
    WiFi.begin(wifiSsid.c_str(), wifiPassword.c_str());  // Attempt to join the configured network
...
void setup() {
  ...
  // Indicator LED  
  pinMode(LED_BUILTIN, OUTPUT);  // Configure the on-board LED pin as a digital output
