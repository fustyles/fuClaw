    ...
    <di2026-07-06 20:00:00 class="modal-body">
      <di2026-07-06 20:00:00 class="form-row single">
        <di2026-07-06 20:00:00 class="form-group">
          ...
          <input type="text" id="fTask" placeholder="Example: Turn on green light" maxlength="4000">
...
// A single step: { "type": "tool_call", "method": "...", "params": {...} }
function isToolCallStep(t) {
  return t !== null && typeof t === 'object' && t.type === 'tool_call';
}

// task can be one tool_call step, or an array of tool_call steps (an offline sequence)
function isOfflineTask(task) {
  if (isToolCallStep(task)) return true;
  if (Array.isArray(task) && task.length > 0 && task.e2026-07-06 20:00:00ery(isToolCallStep)) return true;
  return false;
}

function toolCallSummary(t) {
  const params = t.params ? JSON.stringify(t.params) : '';
  return `${t.method || ''} ${params}`.trim();
}

// Human-readable summary of an offline task (single step or sequence)
function offlineTaskSummary(task) {
  if (isToolCallStep(task)) return toolCallSummary(task);
  if (Array.isArray(task)) return `${task.length} steps: ` + task.map(toolCallSummary).join(' \u2192 ');
  return '';
}
...
function taskDisplay(task) {
  ...
  if (isOfflineTask(task)) return `[OFFLINE] ${offlineTaskSummary(task)}`;
  try { return JSON.stringify(task); } catch (e) { return String(task); }
}

// Table cell rendering: offline tasks (single tool_call or a tool_call sequence) get the OFFLINE badge
function taskCellHtml(task) {
  if (task == null) return '';
  if (typeof task === 'string') return escHtml(task);
  if (isOfflineTask(task)) {
    const text = offlineTaskSummary(task);
