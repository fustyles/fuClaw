...
Ported to ESP32-S3 by: Claude (Anthropic), 2026-06-21
Original platform: Realtek Ameba Pro2 (AMB82-mini / HUB 8735 Ultra)
------------------------------------------------------------
Version
------------------------------------------------------------
...
------------------------------------------------------------
ESP32-S3 PORTING NOTES (read this first)
------------------------------------------------------------
Only the platform-dependent layers below were changed. The
overall architecture, tool routing, Gemini prompt logic, and
business logic are UNCHANGED from the original Ameba Pro2
version.

Changed:
- WiFi.h / WiFiSSLClient      -> ESP32 WiFi.h / WiFiClientSecure
                                 (insecure mode, no cert pinning,
                                 matching original behavior)
- WiFi AP+STA concurrent mode -> WiFi.mode(WIFI_AP_STA) + softAP()
- AmebaFatFS (fs.*)           -> SD_MMC (SD_MMC.* / File)
- rtc.h hardware RTC          -> NTP time sync (configTime) +
                                 time.h, ESP32 internal RTC
- VideoStream / Camera.*      -> esp_camera.h (esp_camera_fb_get /
                                 esp_camera_fb_return)
- AmebaServo                  -> ESP32Servo (Servo class)
- Base64.h (base64_enc_len /  -> local Base64 helper functions
  base64_encode)                 with identical signatures
                                 (no external library needed)
- NVIC_SystemReset()          -> ESP.restart()

NOT changed (kept exactly as-is, API-compatible):
- ArduinoJson, FreeRTOS/task.h, DHT library
- All Gemini / Telegram / LINE HTTP request-building logic
- All tool dispatch, scheduling, and memory persistence logic
- GPIO digitalWrite/digitalRead/analogWrite/analogRead logic

Hardware used for this port:
- Board   : ESP32-S3-WROOM-CAM (OV2640-class camera module)
- Storage : SD_MMC (4-bit or 1-bit mode depending on board wiring)
- Servo   : SG90 (pin kept as a placeholder, confirm wiring)
- DHT11   : pin kept as a placeholder, confirm wiring

GPIO pin numbers below are carried over unchanged from the
original Ameba Pro2 mapping table. ESP32-S3-WROOM-CAM boards
vary by manufacturer (camera pins, SD_MMC pins, free GPIOs all
differ), so before flashing you MUST verify every pin against
your specific board's schematic. Search "ESP32-S3-WROOM-CAM
pinout <vendor>" if unsure. Places needing your attention are
marked with "// TODO(ESP32-S3):" comments throughout this file.
------------------------------------------------------------
Overview
------------------------------------------------------------
fuClaw is an embedded multimodal AI agent framework, originally
built for Realtek Ameba Pro2 devices and ported here to run on
...
HUB 8735 Ultra (original)
- Green LED : GPIO 25
- Blue LED  : GPIO 26
- Fill LED  : GPIO 13
- Button    : GPIO 12 (input only, active-low)

External Modules (original mapping, kept as placeholders)
...
- DHT11 Sensor         : GPIO 20 (placeholder, confirm wiring)

// TODO(ESP32-S3): On ESP32-S3-WROOM-CAM boards, GPIOs 1, 2, 11,
// 12, 13, 20 etc. may be reserved by the camera (XCLK, PCLK,
// VSYNC, HREF, D0-D7, SIOD, SIOC) or by SD_MMC (CLK, CMD, D0-D3).
// Re-map any conflicting pins to free GPIOs on your specific
// board before wiring the servo, DHT11, or other peripherals.
...
// TODO(ESP32-S3): confirm a free GPIO on your ESP32-S3-WROOM-CAM board.
// GPIO 24/25 do not exist on ESP32-S3; this is a placeholder pin only.
int ledPin = 2;    // placeholder, was AMB82-mini:24 / HUB 8735 Ultra:25
...
// ---------------------------------------------------------------------
// Local Base64 helper (drop-in replacement for Ameba's Base64.h)
// Provides base64_enc_len() / base64_encode() with the same signatures
// used throughout this file, so no call sites needed to change.
// ---------------------------------------------------------------------
...
int base64_encode(char *output, char *input, int inputLen) {
  int i = 0, j = 0;
  while (i < inputLen) {
    uint32_t octet_a = i < inputLen ? (uint8_t)input[i++] : 0;
    uint32_t octet_b = i < inputLen ? (uint8_t)input[i++] : 0;
    uint32_t octet_c = i < inputLen ? (uint8_t)input[i++] : 0;
  ...
  // Pad output for any remaining bytes (matches standard base64 padding)
  int mod = inputLen % 3;
  if (mod == 1) {
    output[j - 1] = '=';
    output[j - 2] = '=';
...
// ---------------------------------------------------------------------
// Camera (ESP32 esp_camera driver replaces Ameba VideoStream/Camera.*)
// TODO(ESP32-S3): camera_pins.h below uses a generic ESP32-S3-WROOM-CAM
// pin layout. If your board does not match, replace these pin defines
// with the ones from your board vendor's example sketch.
// ---------------------------------------------------------------------
#include "esp_camera.h"

// TODO(ESP32-S3): verify these against your specific board's schematic.
...
bool initCamera() {
  ...
  // Matches the original VideoSetting(320, 240, ...) resolution.
  ...
  if (err != ESP_OK) {
    Serial.printf("[DEBUG] Camera init failed, error 0x%x\n", err);
    return false;
  }
  return true;
}
...
#include "DHT.h"
// TODO(ESP32-S3): confirm DHTPIN is free on your board (not used by the
// camera or SD_MMC). Placeholder kept from the original Ameba mapping.
#define DHTPIN 20
...
// ESP32-S3 PORT: AmebaServo& -> Servo& (ESP32Servo library). The
// attach()/attached()/write() API is identical, so the function body
...
void setup() {
  ...
  // ESP32-S3 PORT: VideoSetting/Camera.configVideoChannel/videoInit/
  // channelBegin -> single esp_camera_init() call via initCamera().
  if (!initCamera()) {
