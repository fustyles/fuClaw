...
void llmChatReset() {
  ...
  if (xSemaphoreTake(stateMutex, MUTEX_TIMEOUT_TICKS) == pdTRUE) {
    historicalMessages = "";
    executeToolHistory = "";
    systemContent = buildLlmMessage("user", geminiRole, false) + buildLlmMessage("model", "OK");
    systemContentTools = buildLlmMessage("user", geminiRole + devicesDefinitionFinal + devicesRule + skillsDefinition + toolsDefinition, false) + buildLlmMessage("model", "OK");
    systemContentNoTools = buildLlmMessage("user", geminiRole + devicesDefinitionFinal + devicesRule, false) + buildLlmMessage("model", "OK");
    xSemaphoreGive(stateMutex);
  }
...
//
// NOTE: historicalMessages / systemContent / systemContentTools / systemContentNoTools
// are all still built by buildLlmMessage() in Gemini's shape:
//   {"role": "user"|"model", "parts":[{"text": "..."}]}
// so the rest of the codebase (other functions that share these globals) keeps working
// unchanged. This function just converts that shared "contents" string into OpenAI's
// {"role": "user"|"assistant", "content": "..."} message format right before sending it,
// using ArduinoJson to do the parsing/escaping safely instead of manual string surgery.
String openaiChatRequest(String workId, String message, int tools = 1) {
  ...
  String request = "{\"model\":\""+llmModel+"\",\"messages\":[" + contents + "], \"max_completion_tokens\": " + geminiMaxOutputTokens + ", \"temperature\": " + geminiTemperature + "}";
...
// Mirrors geminiSearchRequest()'s interface/locking, but uses OpenAI's
// Chat Completions endpoint with a fixed search model + web_search_options.
// NOTE: only "gpt-5-search-api" supports web_search_options on /v1/chat/completions
// (gpt-4o-search-preview / gpt-4o-mini-search-preview were retired 2026-07-23).
// This is intentionally independent of the global llmModel setting.
String openaiSearchRequest(String workId, String message, int tools = 1) {
  ...
  String request = "{\"model\":\""+llmModel+"\", \"tools\": [{\"type\": \"web_search\"}], \"input\":[" + contents + "], \"max_output_tokens\": " + geminiMaxOutputTokens + ", \"temperature\": " + geminiTemperature + "}";
...
// Mirrors geminiVisionRequest()'s structure/locking, but uses OpenAI's
// Chat Completions endpoint and its multimodal "content" array format:
//   {"role":"user","content":[{"type":"text","text":"..."},
//                              {"type":"image_url","image_url":{"url":"data:image/jpeg;base64,..."}}]}
// llmModel must be set to a vision-capable OpenAI model (e.g. "gpt-4o" / "gpt-4o-mini" / "gpt-4.1").
String openaiVisionRequest(String workId, String message, bool frames = true) {
  ...
  if (client.connect(myDomain, 443)) {
    ...
    String Data = "{\"model\": \"" + llmModel +
                  "\", \"max_output_tokens\": " + geminiMaxOutputTokens +
                  ", \"temperature\": " + geminiTemperature +
    ...
    if (error) {
      ...
      responseText = "[DEBUG] OpenAI API (Vision) Error: " + doc["error"]["message"].as<String>();
      Serial.println(responseText);
	  responseText = "OpenAI API (Vision) Error";
    } 
    else {
      responseText = "Unexpected response from OpenAI Vision.";
    }
...
void setup() {
  ...
  systemContent = buildLlmMessage("user", geminiRole, 0) + buildLlmMessage("model", "OK");
  systemContentTools = buildLlmMessage("user", geminiRole + devicesDefinitionFinal + devicesRule + skillsDefinition + toolsDefinition, 0) + buildLlmMessage("model", "OK");
  systemContentNoTools = buildLlmMessage("user", geminiRole + devicesDefinitionFinal + devicesRule, 0) + buildLlmMessage("model", "OK");  
