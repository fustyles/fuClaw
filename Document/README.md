...
|---------|---------|---------|------------|
| **系統提示詞固定開銷** | 每次呼叫都完整送出工具定義 Schema（現已擴充至數十個工具方法），這是**每一輪對話都重複支付**的固定成本，通常遠大於單輪歷史對話的增量 | 高 | 依 device.md 實際配置裁剪工具清單；導入複雜度分級的兩段式呼叫路由（詳見 12.2 方向 5） |
...
研究方向：在正式呼叫前，先用一次極輕量的呼叫判斷「這輪對話需要哪一組工具」，
        只把對應那一組的 Schema 送進正式呼叫，而非每次都送全部。

設計要點：

1. 分類呼叫必須「輕」，否則多一次呼叫反而更貴：
   - 只帶「角色 + 分類指令 + 這一則使用者訊息」
   - 不帶歷史（historicalMessages）、不帶完整工具 Schema、
     不帶 device.md / skill.md 全文
   - 強制輸出格式壓到最小：只回一個分類代碼，不含任何解釋文字
   - 分類呼叫本身不寫入 historicalMessages，避免歷史又被灌入額外雜訊

2. 分類粒度對應既有工具分組，而非單純「有無工具」的二分法：
...
4. 安全性優先於省錢（最重要的原則）：
   ...
   而非「當作純聊天處理」。
   理由：分類器誤判成聊天 → 該執行的硬體動作被自然語言回覆掉、
        什麼都沒發生，且使用者不一定會馬上發現，
        這比「多花點 token」嚴重得多，違反 9.2 節「安全覆蓋」的核心精神
        （對歧義情況應選擇暫停或從寬處理，而非省略執行判斷）。
...
| 比較項目 | 現行單一呼叫 | 兩段式呼叫路由 |
|---------|------------|--------------|
| 呼叫次數 | 1（或含工作流程評估 1～2 次） | 2～3（多一次輕量分類） |
| 工具 Schema 傳輸量 | 每次全額（全部工具） | 依分類結果，通常僅單一分組 |
| 純聊天情境的相對成本 | 高（仍全額夾帶工具 Schema） | 低（分類後可完全略過工具 Schema） |
| 硬體控制情境的相對成本 | 高 | 略高於單一呼叫（多一次分類，但工具 Schema 本身可能更精簡） |
| 誤判風險 | 無（本來就帶完整工具） | 有，需以「不確定時從寬」原則防呆 |
| 實作複雜度 | 低 | 中（需設計分類 Schema、本地預判規則、安全預設） |

何時划算：使用者互動中「純聊天／問候」占比越高，此架構的節省效果越明顯；
         若裝置幾乎每輪都在操作硬體，兩段式呼叫的分類開銷可能抵銷掉節省的部分，
         需要實測比較兩者的實際 token 帳單再決定是否導入。
```
...
進階問題 5：
  若導入 12.2 節「方向 5：兩段式呼叫路由」，分類呼叫誤將一則實際要求硬體動作的
  訊息判定為 CHAT（純聊天），系統會產生什麼後果？使用者可能多久之後才會發現？
  相較於「每次都送完整工具 Schema、多花一點 token」，這種誤判的代價是否對稱？
  如果要替這個分類機制設計測試案例，你會優先測哪些「容易被誤判」的邊界語句？
```
